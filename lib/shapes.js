...
class Shape {
    constructor(bgColor, textColor, text) {
        // Size same for all children; other variables will come from prompt answers
...
class Circle extends Shape {
    ...
    // Typical indentation practices not followed to avoid error in output format; template literal filled in with prompt answers as variables to create svg
    render() {
        return `${this.size}
...
// Triangle class requires same params as parent
class Triangle extends Shape {
    constructor(bgColor, textColor, text) {
        super(bgColor, textColor, text);
    }

    // Typical indentation practices not followed to avoid error in output format; font placement slightly different
    render() { 
        return `${this.size}
...
// Square class requires same params as parent
class Square extends Shape {
    constructor(bgColor, textColor, text) {
        super(bgColor, textColor, text);
    }

    // Typical indentation practices not followed to avoid error in output format; font placement also different
    render() {
        return `${this.size}
